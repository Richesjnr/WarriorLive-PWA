...
export default function App() {
  ...
  // Admin Detection
  const isAdmin = authRole === 'admin' || authStatus === 'guest';
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 flex flex-col antialiased">
            ...
            <div className="flex items-center gap-2 md:gap-4 shrink-0">
              ...
              <nav className="hidden xl:flex items-center gap-1 overflow-x-auto no-scrollbar">
                {((authRole === 'patient' || isAdmin) ? [
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap ${
                        isActive
                ...
                <button
                  ...
                  className="ml-2 px-3 py-2 bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/30 dark:hover:bg-rose-900/50 text-rose-600 dark:text-rose-400 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer border border-rose-200 dark:border-rose-800/50 whitespace-nowrap"
                >
                ...
                <button
                  ...
                  className="ml-1 px-3 py-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer border border-slate-200 dark:border-slate-700 whitespace-nowrap"
                >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-600 dark:text-slate-400 xl:hidden cursor-pointer"
                >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="xl:hidden border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 space-y-2 flex flex-col z-20 sticky top-[57px] shadow-sm max-h-[80vh] overflow-y-auto"
          >
            {((authRole === 'patient' || isAdmin) ? [
              { route: UiNavigationRoute.DASHBOARD, label: 'Dashboard', icon: Activity },
              { route: UiNavigationRoute.PASSPORT, label: 'Health Passport', icon: Shield },
      ...
      <div className="fixed bottom-20 md:bottom-4 left-4 z-50 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm p-3 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 flex flex-col gap-2 text-xs w-48">
        ...
        <button onClick={() => { setAuthRole('patient'); setAuthStatus('guest'); setActiveTab(UiNavigationRoute.DASHBOARD); }} className={`px-2 py-1.5 rounded text-left flex items-center gap-2 transition-colors cursor-pointer ${authRole === 'patient' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
          <User className="w-3.5 h-3.5" /> Patient View
        </button>
        <button onClick={() => { setAuthRole('professional'); setAuthStatus('guest'); setActiveTab(UiNavigationRoute.DOCTOR); }} className={`px-2 py-1.5 rounded text-left flex items-center gap-2 transition-colors cursor-pointer ${authRole === 'professional' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300' : 'bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
          <Stethoscope className="w-3.5 h-3.5" /> Provider View
        </button>
        <button onClick={() => { setAuthRole('admin'); setAuthStatus('guest'); setActiveTab(UiNavigationRoute.ADMIN); }} className={`px-2 py-1.5 rounded text-left flex items-center gap-2 transition-colors cursor-pointer ${authRole === 'admin' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/50 dark:text-rose-300' : 'bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
